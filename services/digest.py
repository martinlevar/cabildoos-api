...
def _arco_svg_bg() -> str:
    """Genera el arco de puntos de colores (identidad visual CabildoOS) como data URI."""
    W, H = 580, 300
    cx, cy = 290, 360  # centro debajo del canvas para que los arcos suban
    colores = ["#e63946", "#f4a261", "#e9c46a", "#4ade80", "#60a5fa", "#a78bfa", "#e63946", "#f4a261"]
    parts = [f'<rect width="{W}" height="{H}" fill="#0a0f1e"/>']
    for i, radio in enumerate(range(90, 530, 50)):
        n_dots = max(18, int(math.pi * radio / 9))
        for j in range(n_dots):
            ...
            if -4 <= x <= W + 4 and -4 <= y <= H + 4:
                ...
                parts.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="2.8" fill="{color}" opacity="0.9"/>')
    svg = (
